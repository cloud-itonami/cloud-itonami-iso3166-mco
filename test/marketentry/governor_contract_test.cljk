  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems MC"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems MC" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest rci-clearance-missing-is-held-and-unoverridable
  (testing "activity requiring administrative authorization, but only a declaration-receipt on file -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:rci-clearance-missing} (-> (store/ledger db) last :basis)))
...
(deftest fiscal-declaration-unverified-is-held-and-unoverridable
  (testing "unverified Direction des Services Fiscaux declaration when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:fiscal-declaration-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
