...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (MCO, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up rci-clearance-missing) ==")
    ...
    (println "== filing/submit eng-4 (rci-clearance-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up fiscal-declaration-unverified) ==")
    ...
    (println "== filing/submit eng-5 (fiscal-declaration-unverified -> HARD hold) ==")
