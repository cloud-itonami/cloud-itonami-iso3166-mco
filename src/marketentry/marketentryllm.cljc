(ns marketentry.marketentryllm
  ...
  the MCO public-sector market-entry compliance actor.
  ...
  cited), never a committed record or a real RCI registration
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world RCI registration submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けRCI登録提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "requires-administrative-authorization?=" (:requires-administrative-authorization? e)
                        " has-administrative-authorization?=" (:has-administrative-authorization? e)
                        " has-declaration-receipt?=" (:has-declaration-receipt? e)
     ...
     :confidence (if (and e
                          (if (:requires-administrative-authorization? e)
                            (:has-administrative-authorization? e)
                            (:has-declaration-receipt? e))
                          (or (not (:requires-fiscal-declaration? e))
                              (:fiscal-declaration-verified? e)))
                   0.9 0.3)}))
